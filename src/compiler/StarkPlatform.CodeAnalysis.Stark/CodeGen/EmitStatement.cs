    ...
    internal partial class CodeGenerator
    {
        ...
        private static bool CanPassToBrfalse(TypeSymbol ts)
        {
            ...
            switch (tc)
            {
                ...
                case StarkPlatform.Cci.PrimitiveTypeCode.NotPrimitive:
                    // if this is a generic type param, verifier will want us to box
                    // EmitCondBranch knows that
                    return ts.IsReferenceType;

                default:
                    Debug.Assert(tc != StarkPlatform.Cci.PrimitiveTypeCode.Invalid);
                    Debug.Assert(tc != StarkPlatform.Cci.PrimitiveTypeCode.Void);
        ...
        private static BoundExpression TryReduce(BoundBinaryOperator condition, ref bool sense)
        {
            ...
            bool isBool = nonConstType.PrimitiveTypeCode == StarkPlatform.Cci.PrimitiveTypeCode.Boolean;
...
oneMoreTime:
        ...
        private void EmitStringCompareAndBranch(LocalOrParameter key, SyntaxNode syntaxNode, ConstantValue stringConstant, object targetLabel, StarkPlatform.Cci.IReference stringEqualityMethodRef)
        {
