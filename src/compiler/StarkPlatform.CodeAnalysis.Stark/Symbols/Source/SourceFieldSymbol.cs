...
namespace StarkPlatform.CodeAnalysis.Stark.Symbols
{
    ...
    internal abstract class SourceFieldSymbolWithSyntaxReference : SourceFieldSymbol
    {
        ...
        private ConstantValue _lazyConstantEarlyDecodingValue = StarkPlatform.CodeAnalysis.ConstantValue.Unset;
        private ConstantValue _lazyConstantValue = StarkPlatform.CodeAnalysis.ConstantValue.Unset;
        ...
        internal sealed override ConstantValue GetConstantValue(ConstantFieldsInProgress inProgress, bool earlyDecodingWellKnownAttributes)
        {
            ...
            if (value != StarkPlatform.CodeAnalysis.ConstantValue.Unset)
            {
                return value;
            }
            ...
            if (!inProgress.IsEmpty)
            {
                ...
                return StarkPlatform.CodeAnalysis.ConstantValue.Unset;
            }
        ...
        internal ImmutableHashSet<SourceFieldSymbolWithSyntaxReference> GetConstantValueDependencies(bool earlyDecodingWellKnownAttributes)
        {
            ...
            if ((builder.Count == 0) &&
                (value != null) &&
                !value.IsBad &&
                (value != StarkPlatform.CodeAnalysis.ConstantValue.Unset) &&
        ...
        {
            if (this.GetLazyConstantValue(earlyDecodingWellKnownAttributes) != StarkPlatform.CodeAnalysis.ConstantValue.Unset)
            {
                return;
            }
        ...
        private void SetLazyConstantValue(
            ...
            bool startsCycle)
        {
            Debug.Assert(value != StarkPlatform.CodeAnalysis.ConstantValue.Unset);
            Debug.Assert((GetLazyConstantValue(earlyDecodingWellKnownAttributes) == StarkPlatform.CodeAnalysis.ConstantValue.Unset) ||
                (GetLazyConstantValue(earlyDecodingWellKnownAttributes) == value));

            if (earlyDecodingWellKnownAttributes)
            {
                Interlocked.CompareExchange(ref _lazyConstantEarlyDecodingValue, value, StarkPlatform.CodeAnalysis.ConstantValue.Unset);
            }
            else
            {
                if (Interlocked.CompareExchange(ref _lazyConstantValue, value, StarkPlatform.CodeAnalysis.ConstantValue.Unset) == StarkPlatform.CodeAnalysis.ConstantValue.Unset)
                {
#if REPORT_ALL
