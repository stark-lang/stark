...
namespace StarkPlatform.CodeAnalysis.Stark
{
    internal class CSharpSyntaxFactsService : AbstractSyntaxFactsService, ISyntaxFactsService
    {
        ...
        {
            return
                node.IsParentKind(SyntaxKind.ImportDirective) &&
                ((ImportDirectiveSyntax)node.Parent).Name == node;
        }
        ...
        {
            return node.IsKind(SyntaxKind.ImportDirective) ||
                   node.IsKind(SyntaxKind.ExternAliasDirective);
        }

        public bool IsGlobalAttribute(SyntaxNode node)
        {
            return node.IsKind(SyntaxKind.Attribute) && ((AttributeSyntax)node).Target?.Identifier.Kind() == SyntaxKind.AssemblyKeyword;
        }
        ...
        {
            return ((LocalDeclarationStatementSyntax)localDeclarationStatement).Declaration == declarator;
        }
        ...
        public SyntaxNode GetVariablesOfLocalDeclarationStatement(SyntaxNode node)
