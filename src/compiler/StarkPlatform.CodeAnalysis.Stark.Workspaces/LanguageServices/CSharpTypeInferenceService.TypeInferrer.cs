        ...
        private class TypeInferrer : AbstractTypeInferrer
        {
            ...
            protected override IEnumerable<TypeInferenceInfo> InferTypesWorker_DoNotCallDirectly(int position)
            {
                ...
                switch (parent)
                {
                    ...
                    case AttributeSyntax attributeDeclaration: return InferTypeInAttributeDeclaration(attributeDeclaration, token);
            ...
            private IEnumerable<TypeInferenceInfo> InferTypeInAttributeDeclaration(AttributeSyntax attributeDeclaration, SyntaxToken? previousToken)
            {
                // If we have a position, then it has to be after the open bracket.
                if (previousToken.HasValue && previousToken.Value != attributeDeclaration.AtToken)
                {
                    return SpecializedCollections.EmptyEnumerable<TypeInferenceInfo>();
                }
            ...
            private IEnumerable<TypeInferenceInfo> InferTypeInForEachStatement(ForStatementSyntax forEachStatementSyntax, ExpressionSyntax expressionOpt = null, SyntaxToken? previousToken = null)
            {
                ...
                // foreach (int v = Goo())
                var variableTypes = GetTypes(forEachStatementSyntax.Expression);
            ...
            private IEnumerable<TypeInferenceInfo> InferTypeInIfStatement(IfStatementSyntax ifStatement, SyntaxToken? previousToken = null)
            {
                ...
                if (previousToken.HasValue && previousToken.Value != ifStatement.IfKeyword)
                {
                    return SpecializedCollections.EmptyEnumerable<TypeInferenceInfo>();
                }
