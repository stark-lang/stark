...
namespace StarkPlatform.CodeAnalysis.Stark.Formatting
{
    internal partial class CSharpTriviaFormatter : AbstractTriviaFormatter
    {
        ...
        protected override SyntaxTrivia CreateEndOfLine()
        {
            if (_newLine == default)
            {
                var text = this.Context.OptionSet.GetOption(FormattingOptions.NewLine, LanguageNames.Stark);
        ...
        private bool TryFormatMultiLineCommentTrivia(LineColumn lineColumn, SyntaxTrivia trivia, out SyntaxTrivia result)
        {
            ...
            if (indentationDelta != 0)
            {
                var multiLineComment = trivia.ToFullString().ReindentStartOfXmlDocumentationComment(
                    ...
                    indentation,
                    indentationDelta,
                    this.OptionSet.GetOption(FormattingOptions.UseTabs, LanguageNames.Stark),
                    this.OptionSet.GetOption(FormattingOptions.TabSize, LanguageNames.Stark),
                    this.OptionSet.GetOption(FormattingOptions.NewLine, LanguageNames.Stark));
