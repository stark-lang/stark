    ...
    internal static partial class CastExpressionSyntaxExtensions
    {
            ...
            out bool parentIsIsOrAsExpression)
        {
            ...
            if (parentNode.IsKind(SyntaxKind.IsExpression) ||
                parentNode.IsKind(SyntaxKind.AsOptExpression))
            {
        ...
        {
            var leadingTrivia = node.Expression.GetLeadingTrivia()
                .Concat(node.Expression.GetTrailingTrivia())
                .Concat(node.AsKeyword.LeadingTrivia)
                .Concat(node.AsKeyword.TrailingTrivia)
