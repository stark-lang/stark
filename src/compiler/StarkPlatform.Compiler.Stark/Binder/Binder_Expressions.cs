...
namespace StarkPlatform.Compiler.Stark
{
    ...
    internal partial class Binder
    {
        ...
        private BoundExpression BindRefType(RefTypeExpressionSyntax node, DiagnosticBag diagnostics)
        {
            ...
            TypeSymbol typedReferenceType = this.Compilation.GetSpecialType(SpecialType.System_TypedReference);
            TypeSymbol typeType = this.Compilation.GetWellKnownType(WellKnownType.core_Type);
            HashSet<DiagnosticInfo> useSiteDiagnostics = null;
            Conversion conversion = this.Conversions.ClassifyConversionFromExpression(argument, typedReferenceType, ref useSiteDiagnostics);
            diagnostics.Add(node, useSiteDiagnostics);
            if (!conversion.IsImplicit || !conversion.IsValid)
        ...
        private BoundExpression BindRangeExpression(RangeExpressionSyntax node, DiagnosticBag diagnostics)
        {
            ...
            TypeSymbol rangeType = GetWellKnownType(WellKnownType.core_Range, diagnostics, node);
        ...
        private BoundExpression BindRangeExpressionOperand(ExpressionSyntax operand, DiagnosticBag diagnostics, bool reduceByOne)
        {
            ...
            TypeSymbol indexType = GetWellKnownType(WellKnownType.core_Index, diagnostics, operand);
        ...
        private TypeSymbol GetStackAllocType(SyntaxNode node, TypeSymbolWithAnnotations elementType, DiagnosticBag diagnostics, out bool hasErrors)
        {
            ...
            if (inLegalPosition && !node.IsVariableDeclarationInitialization())
            {
                ...
                var spanType = GetWellKnownType(WellKnownType.core_Span_T, diagnostics, node);
            ...
            DiagnosticBag diagnostics)
        {
            ...
            if (!hasErrors)
            {
                ...
                Debug.Assert(Flags.Includes(Stark.BinderFlags.ObjectInitializerMember));
