        ...
        Cci.ISpecializedNestedTypeReference
    {
        ...
        Cci.ITypeReference Cci.ITypeDefinition.GetBaseClass(EmitContext context)
        {
            ...
            if (this.IsScriptClass)
            {
                ...
                baseType = this.ContainingAssembly.GetSpecialType(StarkPlatform.Compiler.SpecialType.System_Object);
            }
            ...
            foreach (var member in this.GetMembers())
            {
                if (member.Kind == SymbolKind.Method)
                {
                    ...
                    if (explicitImplementations.Length != 0)
                    {
                        ...
                        {
                            yield return new StarkPlatform.Cci.MethodImplementation(method, moduleBeingBuilt.TranslateOverriddenMethodReference(implemented, (CSharpSyntaxNode)context.SyntaxNodeOpt, context.Diagnostics));
                        }
                    }
                    ...
                    if (method.RequiresExplicitOverride())
                    {
                        ...
                        yield return new StarkPlatform.Cci.MethodImplementation(method, moduleBeingBuilt.TranslateOverriddenMethodReference(method.OverriddenMethod, (CSharpSyntaxNode)context.SyntaxNodeOpt, context.Diagnostics));
                    }
                    else if (method.MethodKind == MethodKind.Destructor && this.SpecialType != SpecialType.System_Object)
                    {
                        ...
                        TypeSymbol objectType = this.DeclaringCompilation.GetSpecialType(Compiler.SpecialType.System_Object);
                        foreach (Symbol objectMember in objectType.GetMembers(WellKnownMemberNames.DestructorName))
                        {
                            ...
                            {
                                yield return new StarkPlatform.Cci.MethodImplementation(method, moduleBeingBuilt.TranslateOverriddenMethodReference(objectMethod, (CSharpSyntaxNode)context.SyntaxNodeOpt, context.Diagnostics));
                            }
                        }
                    }
                }
        ...
        {
            PEModuleBuilder moduleBeingBuilt = (PEModuleBuilder)context.Module;
            var builder = ArrayBuilder<StarkPlatform.Cci.ITypeReference>.GetInstance();
