...
namespace StarkPlatform.Compiler.Stark.Symbols
{
    ...
    internal abstract class SourceFieldSymbolWithSyntaxReference : SourceFieldSymbol
    {
        ...
        private ConstantValue _lazyConstantEarlyDecodingValue = StarkPlatform.Compiler.ConstantValue.Unset;
        private ConstantValue _lazyConstantValue = StarkPlatform.Compiler.ConstantValue.Unset;
        ...
        internal sealed override ConstantValue GetConstantValue(ConstantFieldsInProgress inProgress, bool earlyDecodingWellKnownAttributes)
        {
            ...
            if (value != StarkPlatform.Compiler.ConstantValue.Unset)
            {
                return value;
            }
            ...
            if (!inProgress.IsEmpty)
            {
                ...
                return StarkPlatform.Compiler.ConstantValue.Unset;
            }
        ...
        internal ImmutableHashSet<SourceFieldSymbolWithSyntaxReference> GetConstantValueDependencies(bool earlyDecodingWellKnownAttributes)
        {
            ...
            if ((builder.Count == 0) &&
                (value != null) &&
                !value.IsBad &&
                (value != StarkPlatform.Compiler.ConstantValue.Unset) &&
        ...
        {
            if (this.GetLazyConstantValue(earlyDecodingWellKnownAttributes) != StarkPlatform.Compiler.ConstantValue.Unset)
            {
                return;
            }
        ...
        private void SetLazyConstantValue(
            ...
            bool startsCycle)
        {
            Debug.Assert(value != StarkPlatform.Compiler.ConstantValue.Unset);
            Debug.Assert((GetLazyConstantValue(earlyDecodingWellKnownAttributes) == StarkPlatform.Compiler.ConstantValue.Unset) ||
                (GetLazyConstantValue(earlyDecodingWellKnownAttributes) == value));

            if (earlyDecodingWellKnownAttributes)
            {
                Interlocked.CompareExchange(ref _lazyConstantEarlyDecodingValue, value, StarkPlatform.Compiler.ConstantValue.Unset);
            }
            else
            {
                if (Interlocked.CompareExchange(ref _lazyConstantValue, value, StarkPlatform.Compiler.ConstantValue.Unset) == StarkPlatform.Compiler.ConstantValue.Unset)
                {
#if REPORT_ALL
