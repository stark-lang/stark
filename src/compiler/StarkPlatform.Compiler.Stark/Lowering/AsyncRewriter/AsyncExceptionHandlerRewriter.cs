...
namespace StarkPlatform.Compiler.Stark
{
    ...
    internal sealed class AsyncExceptionHandlerRewriter : BoundTreeRewriterWithStackGuardWithoutRecursionOnTheLeftOfBinaryOperator
    {
        ...
        private BoundStatement Rethrow(LocalSymbol obj)
        {
            ...
            {
                var ex = _F.SynthesizedLocal(_F.WellKnownType(WellKnownType.core_Exception));
        ...
        private sealed class AwaitCatchFrame
        {
            ...
            {
                if (!_hoistedLocals.Keys.Any(l => l.Name == local.Name && TypeSymbol.Equals(l.Type.TypeSymbol, local.Type.TypeSymbol, TypeCompareKind.ConsiderEverything2)))
                {
