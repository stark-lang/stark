    ...
    internal partial struct Blender
    {
        ...
        private struct Cursor
        {
            ...
            private static int IndexOfNodeInParent(SyntaxNode node)
            {
                ...
                var index = SyntaxNodeOrToken.GetFirstChildIndexSpanningPosition(children, ((Stark.CSharpSyntaxNode)node).Position);
            ...
            public Cursor MoveToFirstChild()
            {
                ...
                if (node.Kind() == SyntaxKind.InterpolatedStringExpression)
                {
                    ...
                    var redToken = new Compiler.SyntaxToken(node.Parent, greenToken, node.Position, _indexInParent);
                    return new Cursor(redToken, _indexInParent);
                }

                if (node.SlotCount > 0)
                {
                    var child = StarkPlatform.Compiler.ChildSyntaxList.ItemInternal(node, 0);
