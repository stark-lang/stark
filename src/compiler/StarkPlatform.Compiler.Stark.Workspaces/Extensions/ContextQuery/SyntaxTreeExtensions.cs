...
namespace StarkPlatform.Compiler.Stark.Extensions.ContextQuery
{
    internal static class SyntaxTreeExtensions
    {
        public static bool IsAttributeNameContext(this SyntaxTree syntaxTree, int position, CancellationToken cancellationToken)
        {
            ...
            // cases:
            //   [ |
            if (token.IsKind(SyntaxKind.AtToken))
            {
                return true;
            }
            ...
            SemanticModel semanticModelOpt = null)
        {
            ...
            {
                var usingDirective = token.GetAncestor<ImportDirectiveSyntax>();
        ...
        public static bool IsUsingAliasContext(this SyntaxTree syntaxTree, int position, CancellationToken cancellationToken)
        {
            ...
            if (token.IsKind(SyntaxKind.EqualsToken) &&
                token.GetAncestor<ImportDirectiveSyntax>() != null)
            {
                return true;
            }
            ...
            this SyntaxTree syntaxTree, int position, SyntaxToken tokenOnLeftOfPosition, CancellationToken cancellationToken)
        {
            ...
            if (Compiler.CSharpExtensions.IsKind(token, SyntaxKind.OutKeyword) &&
                token.Parent.IsKind(SyntaxKind.Argument))
            {
                return true;
            }

            if (Compiler.CSharpExtensions.IsKind(token, SyntaxKind.EqualsToken) &&
            ...
            this SyntaxTree syntaxTree, int position, SyntaxToken tokenOnLeftOfPosition, CancellationToken cancellationToken)
        {
            ...
            // join |
            if (Compiler.CSharpExtensions.IsKind(token, SyntaxKind.JoinKeyword) &&
            ...
            this SyntaxTree syntaxTree, int position, SyntaxToken tokenOnLeftOfPosition, CancellationToken cancellationToken)
        {
            ...
            if (Compiler.CSharpExtensions.IsKind(token, SyntaxKind.OpenParenToken) &&
            ...
            SyntaxToken tokenOnLeftOfPosition, CancellationToken cancellationToken)
        {
                ...
                token.Parent.IsKind(SyntaxKind.EqualsValueClause))
            {
                    ...
                    equalsValue.Parent.IsParentKind(SyntaxKind.VariableDeclaration))
                {
                    ...
                    {
                        return localDeclaration.IsConst;
                    }
                }
            ...
            SemanticModel semanticModelOpt = null)
        {
            ...
            if (!attributes)
            {
                if (token.GetAncestor<AttributeSyntax>() != null)
                {
                    return false;
                }
            }
            ...
            if (token.IsKind(SyntaxKind.EqualsToken))
            {
                if (token.Parent.IsKind(SyntaxKind.NameEquals) &&
                    token.Parent.IsParentKind(SyntaxKind.ImportDirective))
                {
                    return false;
                }
            }
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (token.IsLastTokenOfNode<ExpressionSyntax>())
            {
                ...
                if (Compiler.CSharpExtensions.IsKind(token, SyntaxKind.IdentifierToken) &&
                    token.Parent.IsKind(SyntaxKind.IdentifierName) &&
                    ...
                    token.Parent.IsKind(SyntaxKind.IdentifierName))
                {
                    if (token.Parent.IsParentKind(SyntaxKind.Argument) &&
                        Compiler.CSharpExtensions.IsKind(((ArgumentSyntax)token.Parent.Parent).RefOrOutKeyword, SyntaxKind.OutKeyword))
                    {
                        return false;
                    }
                }
        ...
        public static bool IsCatchFilterContext(this SyntaxTree syntaxTree, int position, SyntaxToken tokenOnLeftOfPosition)
        {
            ...
            if (Compiler.CSharpExtensions.IsKind(token, SyntaxKind.CatchKeyword))
            {
                return true;
            }

            if (Compiler.CSharpExtensions.IsKind(token, SyntaxKind.CloseParenToken) &&
