...
{
    // EditorBrowsable(Never) so that we don't clutter completion list with this type because a user that only has StarkPlatform.Reflection.Metadata 
    ...
    public static class PEReaderExtensions
    {
        ...
        public static MethodBodyBlock GetMethodBody(this PEReader peReader, int relativeVirtualAddress)
        {
            ...
            if (block.Length == 0)
            {
                throw new BadImageFormatException(StarkPlatform.Reflection.Resources.SR.Format(SR.InvalidMethodRva, relativeVirtualAddress));
            }
