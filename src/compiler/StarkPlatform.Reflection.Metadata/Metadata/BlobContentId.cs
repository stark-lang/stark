...
namespace StarkPlatform.Reflection.Metadata
{
    public readonly struct BlobContentId : IEquatable<BlobContentId>
    {
        ...
        public unsafe BlobContentId(byte[] id)
        {
            ...
            if (id.Length != Size)
            {
                throw new ArgumentException(StarkPlatform.Reflection.Resources.SR.Format(SR.UnexpectedArrayLength, Size), nameof(id));
            }
        ...
        public static unsafe BlobContentId FromHash(byte[] hashCode)
        {
            ...
            {
                throw new ArgumentException(StarkPlatform.Reflection.Resources.SR.Format(SR.HashTooShort, minHashSize), nameof(hashCode));
            }
