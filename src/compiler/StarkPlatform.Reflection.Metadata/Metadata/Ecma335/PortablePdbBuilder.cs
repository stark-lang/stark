...
namespace StarkPlatform.Reflection.Metadata.Ecma335
{
    ...
    public sealed class PortablePdbBuilder
    {
        ...
        private static void ValidateTypeSystemRowCounts(ImmutableArray<int> typeSystemRowCounts)
        {
            ...
            {
                throw new ArgumentException(StarkPlatform.Reflection.Resources.SR.Format(SR.ExpectedArrayOfSize, MetadataTokens.TableCount), nameof(typeSystemRowCounts));
            }
            ...
            for (int i = 0; i < typeSystemRowCounts.Length; i++)
            {
                ...
                {
                    throw new ArgumentOutOfRangeException(nameof(typeSystemRowCounts), StarkPlatform.Reflection.Resources.SR.Format(SR.RowCountOutOfRange, i));
                }

                if (((1UL << i) & (ulong)TableMask.ValidPortablePdbExternalTables) == 0)
                {
                    throw new ArgumentException(StarkPlatform.Reflection.Resources.SR.Format(SR.RowCountMustBeZero, i), nameof(typeSystemRowCounts));
                }
            }
        }
