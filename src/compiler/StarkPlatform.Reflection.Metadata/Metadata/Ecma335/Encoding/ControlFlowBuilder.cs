            ...
            foreach (Blob srcBlob in srcBuilder.GetBlobs())
            {
                ...
                while (true)
                {
                    ...
                    if (isShortInstruction && !isShortDistance)
                    {
                        ...
                        throw new InvalidOperationException(StarkPlatform.Reflection.Resources.SR.Format(SR.DistanceBetweenInstructionAndLabelTooBig, branch.OpCode, srcOffset, branchDistance));
                    }
