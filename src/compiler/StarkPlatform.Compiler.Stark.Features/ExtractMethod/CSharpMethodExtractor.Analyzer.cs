    ...
    internal partial class CSharpMethodExtractor : MethodExtractor
    {
        private class CSharpAnalyzer : Analyzer
        {
            ...
            protected override ITypeSymbol GetRangeVariableType(SemanticModel model, IRangeVariableSymbol symbol)
            {
                ...
                if (StarkPlatform.Compiler.Shared.Extensions.ISymbolExtensions.IsErrorType(info.Type))
                {
                    return null;
                }

                return info.Type == null || info.Type.SpecialType == StarkPlatform.Compiler.SpecialType.System_Object
                    ? info.Type
                    : info.ConvertedType;
            }
