...
using StarkPlatform.Compiler.ChangeSignature;
...
namespace StarkPlatform.Compiler.Stark.ChangeSignature
{
    [ExportLanguageService(typeof(AbstractChangeSignatureService), LanguageNames.Stark), Shared]
    internal sealed class CSharpChangeSignatureService : AbstractChangeSignatureService
    {
        ...
        private bool InSymbolHeader(SyntaxNode matchingNode, int position)
        {
            ...
            var lastAttributes = matchingNode.ChildNodes().LastOrDefault(n => n is AttributeSyntax);
