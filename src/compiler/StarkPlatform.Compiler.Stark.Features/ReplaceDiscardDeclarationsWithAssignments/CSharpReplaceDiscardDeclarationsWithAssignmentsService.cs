...
using StarkPlatform.Compiler.ReplaceDiscardDeclarationsWithAssignments;
using Roslyn.Utilities;

namespace StarkPlatform.Compiler.Stark.ReplaceDiscardDeclarationsWithAssignments
{
    [ExportLanguageService(typeof(IReplaceDiscardDeclarationsWithAssignmentsService), LanguageNames.Stark), Shared]
    internal sealed class CSharpReplaceDiscardDeclarationsWithAssignmentsService : IReplaceDiscardDeclarationsWithAssignmentsService
    {
            ...
            foreach (var child in memberDeclaration.DescendantNodes())
            {
                switch (child)
                {
                    case LocalDeclarationStatementSyntax localDeclarationStatement:
                        if (IsDiscardDeclaration(localDeclarationStatement.Declaration))
                        {
        ...
        private sealed class RemoveDiscardHelper : IDisposable
        {
            ...
            private void ProcessDeclarationStatement()
            {
                ...
                {
                    var variable = _localDeclarationStatement.Declaration;
                ...
                var trailingTrivia = _localDeclarationStatement.EosToken.GetAllTrivia();
            ...
            private void GenerateDeclarationStatementForCurrentNonDiscardVariables()
            {
                ...
                //                        SyntaxFactory.VariableDeclaration(SyntaxFactory.Token(SyntaxKind.VarKeyword),  _currentNonDiscardVariables, _localDeclarationStatement.Declaration.Type))
