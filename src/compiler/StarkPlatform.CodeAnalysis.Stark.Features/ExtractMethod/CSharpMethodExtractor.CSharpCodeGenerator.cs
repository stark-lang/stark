        ...
        private abstract partial class CSharpCodeGenerator : CodeGenerator<StatementSyntax, ExpressionSyntax, SyntaxNode>
        {
            ...
            private OperationStatus CheckActiveStatements(IEnumerable<StatementSyntax> statements)
            {
                ...
                foreach (var statement in statements)
                {
                    ...
                    if (declStatement.Declaration.Initializer != null)
                    {
                        // found one
                        return OperationStatus.Succeeded;
                    }
                ...
                foreach (var statement in statements)
                {
                    ...
                    // variable is initialized at decl.
                    {
                        ...
                        if (variableToRemoveMap.HasSyntaxAnnotation(variableDeclaration))
                        {
                            ...
                            {
                                SyntaxToken identifier = ApplyTriviaFromDeclarationToAssignmentIdentifier(declarationStatement, true, variableDeclaration);
                    ...
                    if (list.Count > 0)
                    {
                        yield return SyntaxFactory.LocalDeclarationStatement(list[0], declarationStatement.EosToken.WithPrependedLeadingTrivia(triviaList));
                        triviaList.Clear();
                    }
                ...
                CancellationToken cancellationToken)
            {
                ...
                return SyntaxFactory.LocalDeclarationStatement(
                    SyntaxFactory.VariableDeclaration(SyntaxFactory.Token(SyntaxKind.VarKeyword), SyntaxFactory.Identifier(variable.Name), typeNode, equalsValueClause));
            }
