        ...
        private class PostProcessor
        {
                ...
                Dictionary<ITypeSymbol, List<LocalDeclarationStatementSyntax>> map)
            {
                foreach (var keyValuePair in map)
                {
                    ...
                    foreach (var loc in keyValuePair.Value)
                    {
                        yield return loc;
                    }
                }
            ...
            {
                if (statement.Declaration.Initializer != null)
                {
                    return true;
                }
            ...
            public IEnumerable<StatementSyntax> RemoveInitializedDeclarationAndReturnPattern(IEnumerable<StatementSyntax> statements)
            {
                ...
                if (declaration.Declaration == null ||
                    declaration.Declaration.Initializer == null ||
                    declaration.Declaration.Initializer.Value == null ||
                    declaration.Declaration.Initializer.Value is StackAllocArrayCreationExpressionSyntax ||
                ...
                var variableName = declaration.Declaration.Identifier.ToString();
                if (returnStatement.Expression.ToString() != variableName)
                {
                    return statements;
                }

                return SpecializedCollections.SingletonEnumerable<StatementSyntax>(SyntaxFactory.ReturnStatement(declaration.Declaration.Initializer.Value));
            }
            ...
            public IEnumerable<StatementSyntax> RemoveDeclarationAssignmentPattern(IEnumerable<StatementSyntax> statements)
            {
                ...
                var variable = declaration.Declaration.WithInitializer(SyntaxFactory.EqualsValueClause(assignmentExpression.Right));
                return SpecializedCollections.SingletonEnumerable<StatementSyntax>(declaration.WithDeclaration(variable)).Concat(statements.Skip(2));
            }
        }
    }
}
