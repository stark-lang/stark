...
using StarkPlatform.CodeAnalysis.Differencing;
...
namespace StarkPlatform.CodeAnalysis.Stark.EditAndContinue
{
    [ExportLanguageService(typeof(IEditAndContinueAnalyzer), LanguageNames.Stark), Shared]
    internal sealed class CSharpEditAndContinueAnalyzer : AbstractEditAndContinueAnalyzer
    {
            ...
            while (node != rootOpt)
            {
                switch (node.Kind())
                {
                    ...
                    case SyntaxKind.EventFieldDeclaration:
                        ...
                        return ((BaseFieldDeclarationSyntax)node).Declaration;
        ...
        internal override IEnumerable<SyntaxToken> TryGetActiveTokens(SyntaxNode node)
        {
            if (node.IsKind(SyntaxKind.VariableDeclaration))
            {
                ...
                return fieldDeclaration.Modifiers.Concat(variableDeclaration.DescendantTokens()).Concat(fieldDeclaration.EosToken);
            }
        ...
        protected override SyntaxNode FindStatementAndPartner(SyntaxNode declarationBody, int position, SyntaxNode partnerDeclarationBodyOpt, out SyntaxNode partnerOpt, out int statementPart)
        {
            ...
            switch (node.Kind())
            {
                ...
                case SyntaxKind.VariableDeclaration:
                    ...
                    if (partnerOpt != null)
                    {
                        partnerOpt = ((VariableDeclarationSyntax)partnerOpt);
                    }
        ...
        internal static TextSpan GetDiagnosticSpanImpl(SyntaxKind kind, SyntaxNode node, EditKind editKind)
        {
            switch (kind)
            {
                ...
                case SyntaxKind.Attribute:
                    ...
                    if (editKind == EditKind.Update)
                    {
                        return attributeList.Target?.Span ?? attributeList.Span;
                    }
                ...
                case SyntaxKind.IfStatement:
                    ...
                    return TextSpan.FromBounds(ifStatement.IfKeyword.SpanStart, ifStatement.Condition.Span.End);
        ...
        private struct EditClassifier
        {
            ...
            private void ClassifyUpdate(AttributeSyntax oldNode, AttributeSyntax newNode)
            {
            ...
            SyntaxNode newActiveStatement)
        {
            ...
            ReportUnmatchedStatements<FixedStatementSyntax>(diagnostics, match, new[] { (int)SyntaxKind.FixedStatement }, oldActiveStatement, newActiveStatement,
                areEquivalent: AreEquivalentActiveStatements,
                areSimilar: (n1, n2) => DeclareSameIdentifier(n1.Declaration, n2.Declaration));
        ...
        private static bool DeclareSameIdentifier(VariableDeclarationSyntax oldVariable, VariableDeclarationSyntax newVariable)
        {
            return DeclareSameIdentifier(oldVariable.Identifier, newVariable.Identifier);
        }
        
        private static bool DeclareSameIdentifier(SyntaxToken oldVariable, SyntaxToken newVariable)
        {
            return SyntaxFactory.AreEquivalent(oldVariable, newVariable);
        }
        #endregion
    }
}
