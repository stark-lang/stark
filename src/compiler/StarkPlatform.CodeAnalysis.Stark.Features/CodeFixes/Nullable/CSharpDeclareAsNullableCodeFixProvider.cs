...
namespace StarkPlatform.CodeAnalysis.Stark.CodeFixes.DeclareAsNullable
{
    [ExportCodeFixProvider(LanguageNames.Stark, Name = PredefinedCodeFixProviderNames.DeclareAsNullable), Shared]
    internal class CSharpDeclareAsNullableCodeFixProvider : SyntaxEditorBasedCodeFixProvider
    {
        ...
        {
            if (!node.IsKind(SyntaxKind.NullLiteralExpression, SyntaxKind.AsOptExpression))
            {
                return null;
            }
