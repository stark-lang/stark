...
using StarkPlatform.CodeAnalysis.ReplaceMethodWithProperty;

namespace StarkPlatform.CodeAnalysis.Stark.CodeRefactorings.ReplaceMethodWithProperty
{
    [ExportLanguageService(typeof(IReplaceMethodWithPropertyService), LanguageNames.Stark), Shared]
    internal class CSharpReplaceMethodWithPropertyService : AbstractReplaceMethodWithPropertyService, IReplaceMethodWithPropertyService
    {
            ...
            string propertyName, bool nameChanged)
        {
            ...
            var property = SyntaxFactory.PropertyDeclaration(
                ...
                nameToken, getMethodDeclaration.ReturnType, accessorList: null);
        ...
        private static AccessorDeclarationSyntax CreateGetAccessorWorker(GetAndSetMethods getAndSetMethods)
        {
            ...
            if (getMethodDeclaration.ExpressionBody != null)
            {
                return accessor.WithExpressionBody(getMethodDeclaration.ExpressionBody)
                               .WithEosToken(getMethodDeclaration.EosToken);
            }

            if (getMethodDeclaration.EosToken.Kind() != SyntaxKind.None)
            {
                return accessor.WithEosToken(getMethodDeclaration.EosToken);
            }
            ...
            SemanticModel semanticModel, SyntaxGenerator generator, GetAndSetMethods getAndSetMethods)
        {
            ...
            if (setMethodDeclaration.ExpressionBody != null)
            {
                ...
                return accessor.WithExpressionBody(oldExpressionBody.WithExpression(expression))
                               .WithEosToken(setMethodDeclaration.EosToken);
            }

            if (setMethodDeclaration.EosToken.Kind() != SyntaxKind.None)
            {
                return accessor.WithEosToken(setMethodDeclaration.EosToken);
            }
