...
using StarkPlatform.CodeAnalysis.Stark.RemoveUnusedVariable;
...
{
    [ExportLanguageService(typeof(ICodeCleanupService), LanguageNames.Stark), Shared]
    internal class CSharpCodeCleanupService : ICodeCleanupService
    {
        ...
        public EnabledDiagnosticOptions GetEnabledDiagnostics(OptionSet optionSet)
        {
            ...
            {
                if (optionSet.GetOption(option, LanguageNames.Stark))
                {
                    diagnosticSets.AddRange(diagnosticSet);
                }
            }

            return new EnabledDiagnosticOptions(diagnosticSets.ToImmutableArray(), new OrganizeUsingsSet(optionSet, LanguageNames.Stark));
        }
    }
}
