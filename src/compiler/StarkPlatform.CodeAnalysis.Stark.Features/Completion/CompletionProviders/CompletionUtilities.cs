...
namespace StarkPlatform.CodeAnalysis.Stark.Completion.Providers
{
    internal static class CompletionUtilities
    {
        ...
        internal static bool IsTriggerCharacter(SourceText text, int characterPosition, OptionSet options)
        {
            ...
            if (options.GetOption(CompletionOptions.TriggerOnTypingLetters, LanguageNames.Stark) && IsStartingNewWord(text, characterPosition))
            {
                return true;
            }
        ...
        internal static bool IsTriggerAfterSpaceOrStartOfWordCharacter(SourceText text, int characterPosition, OptionSet options)
        {
            ...
            return SpaceTypedNotBeforeWord(ch, text, characterPosition) ||
                (IsStartingNewWord(text, characterPosition) && options.GetOption(CompletionOptions.TriggerOnTypingLetters, LanguageNames.Stark));
        }
